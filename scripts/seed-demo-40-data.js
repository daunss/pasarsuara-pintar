...
// Supplier untuk negosiasi
const suppliers = [
  { name: 'Toko Beras Makmur', phone: '6281234567890' },
  { name: 'Supplier Sayur Segar', phone: '6281234567891' },
  { name: 'Distributor Minyak Goreng', phone: '6281234567892' },
  { name: 'Toko Bumbu Dapur', phone: '6281234567893' },
]

async function seedDemoData() {
  console.log('🌱 Seeding 40 Demo Data untuk daunsnime...\n')

  try {
    // 1. Get user daunsnime
    console.log('Step 1: Mencari user daunsnime...')
    const { data: users, error: userError } = await supabase
      .from('users')
      .select('*')
      .or('email.ilike.%daunsnime%,phone_number.eq.+6285119607506')
      .limit(1)

    if (userError) throw userError
    if (!users || users.length === 0) {
      throw new Error('User daunsnime tidak ditemukan! Pastikan sudah register.')
    }

    const user = users[0]
    console.log(`✓ User ditemukan: ${user.email} (${user.id})`)

    // 2. Seed Inventory (10 items)
    console.log('\nStep 2: Menambahkan 10 inventory items...')
    
    // Hapus inventory lama dulu
    await supabase
      .from('inventory')
      .delete()
      .eq('user_id', user.id)
    ...
    const inventoryData = products.map((product, index) => ({
      ...
      stock_qty: Math.floor(Math.random() * 50) + 10, // 10-60 items
      unit: product.category === 'Minuman' ? 'gelas' : 'porsi',
      min_sell_price: product.price,
      max_buy_price: Math.floor(product.price * 0.7), // 70% dari harga jual
    ...
    // 3. Seed Transactions (25 transactions)
    console.log('\nStep 3: Menambahkan 25 transaksi...')
    
    // Hapus transaksi lama dulu
    await supabase
      .from('transactions')
      .delete()
      .eq('user_id', user.id)
    ...
    // 4. Seed Negotiation Logs (5 negotiations)
    console.log('\nStep 4: Menambahkan 5 negotiation logs...')
    
    // Hapus negotiation lama dulu
    await supabase
    ...
    const negotiations = [
      {
        buyer_id: user.id,
        seller_id: user.id, // Self for demo
